...
void CGraphicTextInstance::Update()
{
	...
	if (m_roText.IsNull())
	{
		Tracef("CGraphicTextInstance::Update - Ʈ  ʾҽϴ\n");
		return;
	}
				...
				for (int i = 0; i < wTextLen; )
				{
					...
					else
					{
                        ...
                        else if (ret == TEXT_TAG_EMOJI_END)
                        {
                            ...
                            char szPath[255];
                            _snprintf(szPath, sizeof(szPath), "stuffs/emoji/%s.tga", retBuf);
...
void CGraphicTextInstance::UpdatePosition(const D3DXVECTOR3& delta)
{
	m_v3Position.x += delta.x;
	m_v3Position.y += delta.y;
	m_v3Position.z += delta.z;
}
