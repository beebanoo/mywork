...
bool CSpeedTreeWrapper::LoadTree(const char* pszSptFile, const BYTE* c_pbBlock = NULL, unsigned int uiBlockSize = 0, unsigned int nSeed, float fSize, float fSizeVariance)
{
	...
	if (m_pSpeedTree->Compute(NULL, nSeed))
	{
		...
		std::string sptFile = std::string(pszSptFile);
		std::string sptFilePath = CFileNameHelper::GetPath(sptFile);
		std::string branchTextureFilename = std::string(m_pTextureInfo->m_pBranchTextureFilename);
		std::string branchTexturePath = CFileNameHelper::NoExtension(branchTextureFilename) + ".dds";
		LoadTexture((sptFilePath + branchTexturePath).c_str(), m_BranchImageInstance);

		if (m_pTextureInfo->m_pSelfShadowFilename != NULL)
		{
			std::string selfShadowFilename = std::string(m_pTextureInfo->m_pSelfShadowFilename);
			std::string shadowTexturePath = CFileNameHelper::NoExtension(selfShadowFilename) + ".dds";
			LoadTexture((sptFilePath + shadowTexturePath).c_str(), m_ShadowImageInstance);
		}
		...
		{
			std::string compositeFilename = std::string(m_pTextureInfo->m_pCompositeFilename);
			std::string compositeTexturePath = CFileNameHelper::NoExtension(compositeFilename) + ".dds";
			LoadTexture((sptFilePath + compositeTexturePath).c_str(), m_CompositeImageInstance);
		}
