...
namespace UI
{
	...
	void CWindow::Clear()
	{
		...
		m_pMaskWindow = NULL;
#endif
		for_each(m_pChildList.begin(), m_pChildList.end(), FClear());
		m_pChildList.clear();
	...
	void CWindow::Render()
	{
		...
		for_each(m_pChildList.begin(), m_pChildList.end(), std::mem_fn(&CWindow::Render));
	}
	...
	long CWindow::UpdateRect()
	{
		...
		m_rect.right = m_rect.left + m_lWidth;
#endif
		for_each(m_pChildList.begin(), m_pChildList.end(), std::mem_fn(&CWindow::UpdateRect));
    ...
    void CAniImageBox::SetRenderingRect(float fLeft, float fTop, float fRight, float fBottom)
    {
        ...
        for_each(m_ImageVector.begin(), m_ImageVector.end(), m_SetRenderingRect);
    }
#else
	void CAniImageBox::SetRenderingRect(float fLeft, float fTop, float fRight, float fBottom)
	{
    ...
    void CAniImageBox::SetRenderingMode(int iMode)
    {
        ...
        for_each(m_ImageVector.begin(), m_ImageVector.end(), m_SetRenderingMode);
    }
#else
	void CAniImageBox::SetRenderingMode(int iMode)
	{
