...
PyObject* evetAddEvent(PyObject* poSelf, PyObject* poArgs)
{
	char* iBuf;
	if (!PyTuple_GetString(poArgs, 0, &iBuf))
		return Py_BuildException();
	int iSkin;
	if (!PyTuple_GetInteger(poArgs, 1, &iSkin))
		return Py_BuildException();
	...
	int iIndex = rpem.RegisterEventSetFromString(buf);
	if (-1 != iIndex)
	{
		rpem.SetVisibleLineCount(iIndex, 30);
		CPythonNetworkStream::Instance().OnScriptEventStart(iSkin, iIndex);
	}
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_MULTILANGUAGE_SYSTEM
		{ "AddEvent",					evetAddEvent,						METH_VARARGS },
#endif
		{ NULL,							NULL,								NULL         },
	};
