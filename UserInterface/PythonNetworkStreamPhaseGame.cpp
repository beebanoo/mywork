...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	else
	{
		...
		if (CHAT_TYPE_SHOUT == kChat.type)
		{
			...
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "GetLocaleString", Py_BuildValue("(is)", kChat.type, buf));
		}
#else
...
bool CPythonNetworkStream::RecvMailboxAddData()
{
	...
	_AddData = new CPythonMail::SMailBoxAddData(p.szFrom, p.szMessage, p.iYang, p.ItemVnum, p.ItemCount, p.alSockets, p.aAttr
...
bool CPythonNetworkStream::SendPostWrite(const char* szName, const char* szTitle, const char* szMessage, const TItemPos& pos, const int iYang)
{
...
bool CPythonNetworkStream::RecvDamageTopInfo()
{
	...
	while (packet.wSize > 0)
	{
		...
		const char* szVictimName = damage.szVictimName;

		DWORD dwVnum = 0;
		if (sscanf(szVictimName, "%u", &dwVnum) == 1)
		{
			const CPythonNonPlayer::TMobTable* pMobTable = CPythonNonPlayer::Instance().GetTable(dwVnum);
			if (pMobTable)
				szVictimName = pMobTable->szLocaleName;
		}

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_DamageTopAddMember", Py_BuildValue("(iisiisi)", 
			damage.bDamageType, iPos, damage.szAttackerName, damage.bAttackerLevel, damage.bAttackerEmpire, szVictimName, damage.dwDamageValue));
