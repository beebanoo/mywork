...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		CGraphicImageInstance * pLang = pTextTail->pLanguageInstance;
#endif
#ifdef ENABLE_TITLE_PERSO
		CGraphicTextInstance* pPTitleInstance = pTextTail->pPTitleTextInstance;
#endif
		if (pMarkInstance && pGuildNameInstance)
		{
			...
			if (pPTitleInstance)
			{
				pMarkInstance->UpdatePosition(D3DXVECTOR2(0, -15));
		...
		if (pPTitleInstance)
		{
			pPTitleInstance->SetPosition(pTextTail->x, pTextTail->y - 15, pTextTail->z);


			pPTitleInstance->Update();
		}
#endif
		...
		if (pLang)
		{
			...
			if (pLevel)
			{
				...
				if (pTitle)
				{
					int iTitleWidth, iTitleHeight;
					pTitle->GetTextSize (&iTitleWidth, &iTitleHeight);
					pLang->SetPosition (pTextTail->x - (iNameWidth / 2) - 4.0f - iTitleWidth - iLevelWidth - pLang->GetWidth(), pTextTail->y - 10);
				}
				if (!pTitle)
				{
					pLang->SetPosition (pTextTail->x - (iNameWidth / 2) - 4.0f - iLevelWidth - pLang->GetWidth(), pTextTail->y - 10);
				}
			}
		}
#endif
...
#endif
#ifdef ENABLE_TITLE_PERSO
		if (pTextTail->pPTitleTextInstance)
		{
			pTextTail->pPTitleTextInstance->Render();
		}
#endif
	}
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
...
#endif
#ifdef ENABLE_TITLE_PERSO
	pTextTail->pPTitleTextInstance = NULL;
#endif
	...
	{
		BYTE c_pszLang = pCharacterInstance->GetLanguage();
		const char* langlist[] = { "cz","de","dk","en","es","fr","gr","hu","it","nl","pl","pt","ro","ru","tr" };
		if (c_pszLang > 15 || c_pszLang == 0)
			c_pszLang = 1;

		char szPath[256];
		sprintf(szPath, "d:/ymir work/ui/language_flag/%s.tga", langlist[c_pszLang -1]);
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	if (pTextTail->pPTitleTextInstance)
	{
		CGraphicTextInstance::Delete(pTextTail->pPTitleTextInstance);
		pTextTail->pPTitleTextInstance = NULL;
	}
#endif
...
#ifdef ENABLE_TITLE_PERSO
void CPythonTextTail::AttachPTitle(DWORD dwVID, const char* pptitle)
{
	if (!bPKTitleEnable)
		return;
	...
	if (!pPTitleInstance)
	{
		pPTitleInstance = CGraphicTextInstance::New();
		pPTitleInstance->SetTextPointer(ms_pFont);
		pPTitleInstance->SetOutline(true);
		pPTitleInstance->SetHorizonalAlign(CGraphicTextInstance::HORIZONTAL_ALIGN_CENTER);
		pPTitleInstance->SetVerticalAlign(CGraphicTextInstance::VERTICAL_ALIGN_BOTTOM);
	}

	pPTitleInstance->SetValue(pptitle);

	DWORD color = 0;
	if (strlen(pptitle) > 12)
	else if (strlen(pptitle) > 9)
	else if (strlen(pptitle) > 6)
	else if (strlen(pptitle) > 3)

	pPTitleInstance->SetColor(color);

	pPTitleInstance->Update();
}

void CPythonTextTail::DetachPTitle(DWORD dwVID)
{
	if (!bPKTitleEnable)
		return;
