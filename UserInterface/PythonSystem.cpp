...
int CPythonSystem::GetEnvironmentMode()
{
	return m_Config.bEnvironmentMode;
}

void CPythonSystem::SetEnvironmentMode(unsigned int envir)
{
	m_Config.bEnvironmentMode = envir;
}
...
bool CPythonSystem::LoadConfig()
{
	...
	if (NULL == (fp = fopen("configs/metin2.cfg", "rt")))
		return false;
	...
	while (fgets(buf, 256, fp))
	{
		...
		else if (!stricmp(command, "ENVIRONMENT_MODE")) 
			m_Config.bEnvironmentMode = atoi(value);
#if defined(ENABLE_FOV_OPTION)
...
bool CPythonSystem::SaveConfig()
{
	...
	if (NULL == (fp = fopen("configs/metin2.cfg", "wt")))
		return false;
	...
	fprintf(fp, "ENVIRONMENT_MODE		%d\n", m_Config.bEnvironmentMode);
#if defined(ENABLE_FOV_OPTION)
	fprintf(fp, "FOV					%.2f\n", m_Config.fFOV);
#endif
	fprintf(fp, "\n");
