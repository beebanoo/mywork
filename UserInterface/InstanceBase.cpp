...
const char * CInstanceBase::GetPTitle()
{
...
#endif
#ifdef ENABLE_TITLE_PERSO
		SetPTitle(c_rkCreateData.m_PTitle.c_str());
#endif
	}
...
{	
	if (__IsMainInstance())
	{
		TraceError("CInstanceBase::PushTCPState ÷̾ ڽſ ̵Ŷ  ȵȴ!");
		return;
	}
...
#ifdef ENABLE_TITLE_PERSO
void CInstanceBase::SetPTitle(const char* ptitle)
{
	m_PTitle = ptitle;
	RefreshTextTailTitle();
}
...
void CInstanceBase::__ClearWeaponRefineEffect()
{
	...
	if (extended_weapon_effect)
	{
		__DetachEffect(extended_weapon_effect);
		__DetachEffect(extended_weapon_effect_left);
		extended_weapon_effect = 0;
		extended_weapon_effect_left = 0;
	}
#endif
}
...
UINT CInstanceBase::__GetRefinedEffect(CItemData* pItem)
{
	...
	case CItemData::ITEM_TYPE_COSTUME:
		...
		const int effect_num = EFFECT_REFINED_NUM;
		static Defined_weapon_enum weapon[effect_num]
		{
			{10152, EFFECT_LEGACY_SWORD},
			{10153, EFFECT_LEGACY_DAGGER},
			{10154, EFFECT_LEGACY_BOW},
			{10155, EFFECT_LEGACY_TWOHAND},
			{10156, EFFECT_LEGACY_BELL},
			{10157, EFFECT_LEGACY_FAN},
			
			{10164, EFFECT_FUSION_SWORD},
			{10165, EFFECT_FUSION_DAGGER},
			{10166, EFFECT_FUSION_BOW},
			{10167, EFFECT_FUSION_TWOHAND},
			{10168, EFFECT_FUSION_BELL},
			{10169, EFFECT_FUSION_FAN},
			{10170, EFFECT_FUSION_SSWORD},
		};

		for (DWORD w = 0; w<effect_num; w++)
		{
			if (weapon[w].Vnum <= pItem->GetIndex() && pItem->GetIndex() <= weapon[w].VnumMax || weapon[w].Vnum == pItem->GetIndex())
			{
				switch (pItem->GetSubType())
				{
					case CItemData::COSTUME_WEAPON_DAGGER:
					...
					case CItemData::COSTUME_WEAPON_FAN:
						if (IsMountingHorse())
						{
							...
							extended_weapon_effect = EFFECT_REFINED + weapon[w].EffectName;
						break;
						break;
						break;
						break;
				...
				if (extended_weapon_effect)
					extended_weapon_effect = __AttachEffect(extended_weapon_effect);
				if (extended_weapon_effect_left)
					extended_weapon_effect_left = __AttachEffect(extended_weapon_effect_left);
			}
		}
		break;
#endif
