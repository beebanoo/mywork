...
#endif
#if defined(ENABLE_EFFECT_DAMAGE_POISON)
	else
	{
#endif
		if (bSelf)
...
void CInstanceBase::__AttachEmpireEffect(DWORD eEmpire)
{
	...
	std::initializer_list<DWORD> vnums_npc = {30308, 30000};
	DWORD race = GetRace();
	if (std::find(vnums_npc.begin(), vnums_npc.end(), race) != vnums_npc.end()) 
	{
		return;
	}
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	std::unordered_set<std::string> promoters = { "DivineBlades", "Kallo", "PaPaG", "VaruGiGi", "FierarulSam", "Blondu", "Bronx" };
	
	if (IsGameMaster())
	{
		if (szName.find("[Dev]") != std::string::npos) 
		{
			sprintf(szText, "|cFF800080|h[Developer]|h|r");
			CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, s_kLevelColor); 
		}
		else if (szName.find("[Own]") != std::string::npos)
		{
			sprintf(szText, "|cFFAA0000|h[Owner]|h|r");
			CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, s_kLevelColor);    
		}
		else if (promoters.find(szName) != promoters.end())
		{
			sprintf(szText, "Lv %d |cFF2EAFE6|h[Promoter]|h|r", level);
			CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, s_kLevelColor);    
		}
		
		else
		{
			sprintf(szText, "|cFF009000|h[Staff]|h|r");
			CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, s_kLevelColor);
		}
	}
	else if (IsPC())
	{
		if (promoters.find(szName) != promoters.end())
		{
			sprintf(szText, "Lv %d |cFF2EAFE6|h[Promoter]|h|r", level);
			CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, s_kLevelColor);    
		}
...
void CInstanceBase::RefreshTextTail()
{
	...
	const char* ptitle = GetPTitle();

	if (!IsPC() || strlen(ptitle) == 0)
		CPythonTextTail::Instance().DetachPTitle(GetVirtualID());
	else
		CPythonTextTail::Instance().AttachPTitle(GetVirtualID(), ptitle);
