...
PyObject* playerGetPointByApply(PyObject* poSelf, PyObject* poArgs)
{
	int applyType;
	if (!PyTuple_GetInteger(poArgs, 0, &applyType))
		return Py_BuildException();

	int iPoint = CPythonPlayer::Instance().GetPointByApply(applyType);
	return Py_BuildValue("i", iPoint);
}
...
PyObject* playerGetPTitle(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();

	return Py_BuildValue("s", pInstance ? pInstance->GetPTitle() : "");
}
#endif
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetPointByApply",			playerGetPointByApply,				METH_VARARGS },
...
#ifdef ENABLE_TITLE_PERSO
		{ "GetPTitle",					playerGetPTitle,					METH_VARARGS },
#endif
	...
	PyObject* poModule = Py_InitModule("__t_2_p_l_r__", s_methods);
