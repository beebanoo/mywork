...
PyObject * systemGetEnvironmentMode(PyObject * poSelf, PyObject * poArgs)
{
	return Py_BuildValue("i", CPythonSystem::Instance().GetEnvironmentMode());
}

PyObject * systemSetEnvironmentMode(PyObject * poSelf, PyObject * poArgs)
{
	int envir;

	if (!PyTuple_GetInteger(poArgs, 0, &envir))
		return Py_BuildException();

	if (envir > 0)
		CPythonSystem::Instance().SetEnvironmentMode(envir);
...
void initsystem()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetEnvironmentMode",			systemGetEnvironmentMode,		METH_VARARGS },
		{ "SetEnvironmentMode",			systemSetEnvironmentMode,		METH_VARARGS },
