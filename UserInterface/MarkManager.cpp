...
bool CGuildMarkManager::GetMarkImageFilename(DWORD imgIdx, std::string & path) const
{
	...
	char buf[64];
	snprintf(buf, sizeof(buf), "data/mark/%s_%lu.tga", m_pathPrefix.c_str(), imgIdx);
	path = buf;
	return true;
}
...
{
	char buf[64];
	snprintf(buf, sizeof(buf), "data/mark/%s_index", m_pathPrefix.c_str());
	FILE * fp = fopen(buf, "r");
...
bool CGuildMarkManager::SaveMarkIndex()
{
	...
	if (fclose(fp) != 0)
	{
		sys_err("MarkManager::SaveMarkData: error closing file.");
		return false;
	}
